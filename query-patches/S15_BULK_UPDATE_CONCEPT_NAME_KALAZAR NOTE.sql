...
UPDATE concept_name SET NAME= 'KATF ( kala-azar treatment failure case)' WHERE concept_name_type='SHORT' AND CONCEPT_ID=(SELECT a.CONCEPT_ID FROM (select * from concept_name WHERE NAME = 'Kalaazar-KATF (kala' AND concept_name_type='FULLY_SPECIFIED' limit 1) a );
...
UPDATE concept_name SET NAME= 'Kalaazar note' WHERE concept_name_type='SHORT' AND CONCEPT_ID=(SELECT a.CONCEPT_ID FROM (select * from concept_name WHERE NAME = 'Kalazar note' AND concept_name_type='FULLY_SPECIFIED' limit 1) a );